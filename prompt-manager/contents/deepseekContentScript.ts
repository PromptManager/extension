...
const handleMessage = (
  ...
  if (!textarea) {
    console.error(
      "Prompt Manager: Could not find ChatGPT textarea. Autofill failed."
    )
    sendResponse({
      success: false,
      error: `Required DOM element (${textareaSelector}) not found`
    })
    return false
  }
  ...
  try {
    ...
    setTimeout(() => {
      if (document.body.contains(textarea)) {
        ...
        console.warn(
          "Prompt Manager: Textarea element disappeared before Enter simulation."
        )
    ...
    console.error(
      "Prompt Manager: Error during prompt fill/send:",
      errorMessage
    )
...
try {
  ...
  console.error(
    "Prompt Manager: Error adding message listener:",
